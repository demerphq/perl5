...
the C<namespace> declaration, C<__NAMESPACE__>, and lexical package aliases
using C<use ... as ...>.  See L<perlnamespace>.
...
from a block.  Each call resumes the block until its next C<generator_yield>,
...
back to the suspended C<generator_yield>.  See L<perlfunc/generator_create
BLOCK> and L<perlgenerator> for details.

The C<generator_running>, C<generator_completed>, C<generator_failed>, and
C<generator_exhausted> predicates are experimental builtins.  They can be
imported individually with C<use builtin>, or together with the feature by
using the C<generator> pragma.
