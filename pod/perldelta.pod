...
=head2 Experimental iterators

The experimental C<iterator> feature creates a callable one-shot iterator
from a block.  Each call resumes the block until its next C<iterator_yield>,
returning one value, and returns an empty list after the block has finished.
See L<perlfunc/iterator_create BLOCK> for details.
