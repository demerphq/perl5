...
block.  Each call resumes the block until its next C<yield>, returning one
value in list or scalar context.  Once the block finishes, calls return an
empty list and the generator cannot be resumed.
...
C<undef> is a valid yielded value.  A generator is a one-shot continuation:
it cannot be resumed while already running, after exhaustion, or after an
uncaught exception.
