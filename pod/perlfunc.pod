...
=item iterator_create BLOCK
X<iterator_create> X<iterator_yield>

=for Pod::Functions create a one-shot iterator

The experimental C<iterator> feature creates a callable iterator from a
block.  Each call resumes the block until its next C<iterator_yield>, returning
exactly one value in list context or that value in scalar context.  The
iterator body runs in the context of the call that resumes it, and its lexical
and localized state is retained across yields.

    use feature 'iterator';

    my $numbers = iterator_create {
        iterator_yield 1;
        iterator_yield 2;
    };
...
empty list and the iterator becomes exhausted.  A later call then raises an
exception; exhaustion is permanent.

An iterator does not accept arguments.  It is a one-shot continuation: it
cannot be resumed while it is already running, after exhaustion, or after an
uncaught exception.  An exception caught by an C<eval> inside the iterator
does not exhaust it; an uncaught exception is propagated by the call that
resumes the iterator and permanently puts it into the failed state.

C<iterator_yield> is only valid lexically inside an C<iterator_create> block.
...
=item iterator_exhausted EXPR
X<iterator_exhausted>

=for Pod::Functions test whether an iterator is exhausted

Returns true if EXPR is an iterator that has completed and returned an empty
list on its final resume.  It returns false for a new or suspended iterator,
and does not resume or advance the iterator.  A failed iterator is not
