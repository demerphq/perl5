...
=for Pod::Functions test whether a generator is exhausted

Returns true if EXPR is a generator that has completed and returned an empty
...
=item generator BLOCK
X<generator> X<yield>

=for Pod::Functions create a one-shot generator

The experimental C<generator> feature creates a callable generator from a
block.  Each call resumes the block until its next C<yield>, returning exactly
one value in list context or that value in scalar context.  The generator body
runs in the context of the call that resumes it, and its lexical and localized
state is retained across yields.
...
A generator does not accept arguments.  It is a one-shot continuation: it
...
C<yield> is only valid lexically inside a C<generator> block.  It cannot be
used in an ordinary subroutine or in callback code such as C<sort>, C<map>,
C<grep>, or a regular-expression code block.
