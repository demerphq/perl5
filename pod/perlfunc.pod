...
resetting the L<C<each>|/each HASH> generator in the process.
...
its own internal generator, accessed by C<each>,
L<C<keys>|/keys HASH>, and L<C<values>|/values HASH>.  The generator is
...
a hash's elements while iterating over it, the effect on the generator is
...
The generator used by C<each> is attached to the hash or array, and is
...
generator location.  All uses of C<each> are also subject to having the
generator reset by any use of C<keys> or C<values> on the same hash or
...
such a loop with the generator already part way through the object, or to
accidentally clobber the generator state during execution of the loop body.
It's easy enough to explicitly reset the generator before starting a loop,
but there is no way to insulate the generator state used by a loop from
the generator state used by anything else that might execute during the
loop body.
...
To avoid these problems resulting from the hash-embedded generator, use a
...
=item generator_create BLOCK
X<generator_create> X<generator_yield>

=for Pod::Functions create a one-shot generator

The experimental C<generator> feature creates a callable generator from a
block.  Each call resumes the block until its next C<generator_yield>, returning
exactly one value in list context or that value in scalar context.  The
generator body runs in the context of the call that resumes it, and its lexical
and localized state is retained across yields.
...
C<undef> is a valid yielded value, and is still one value rather than an empty
list.  When the body finishes without yielding again, that call returns an
empty list and the generator becomes exhausted.  A later call then raises an
exception; exhaustion is permanent.

An generator does not accept arguments.  It is a one-shot continuation: it
cannot be resumed while it is already running, after exhaustion, or after an
uncaught exception.  An exception caught by an C<eval> inside the generator
does not exhaust it; an uncaught exception is propagated by the call that
resumes the generator and permanently puts it into the failed state.

C<generator_yield> is only valid lexically inside an C<generator_create> block.
It cannot be used in an ordinary subroutine or in callback code such as
C<sort>, C<map>, C<grep>, or a regular-expression code block.

This feature is experimental and may change or be removed in a future Perl
release.

=item generator_exhausted EXPR
X<generator_exhausted>

=for Pod::Functions test whether an generator is exhausted

Returns true if EXPR is an generator that has completed and returned an empty
list on its final resume.  It returns false for a new or suspended generator,
and does not resume or advance the generator.  A failed generator is not
exhausted.  This keyword is experimental and requires C<use feature
'generator'>.
...
generator of the HASH or ARRAY (see L<C<each>|/each HASH>) before
yielding the keys.  In
particular, calling C<keys> in void context resets the
generator with no other overhead.
...
There is a now a facility to control the generator for the C<@INC> array
...
the following code
    ...
    push @INC, sub {
        ...
        undef $INC; # reset the $INC generator so we
...
the left of the generator, and so the warning would only be generated on
...
ARRAY's internal generator (see L<C<each>|/each HASH>) before yielding the
values.  In particular,
calling C<values> in void context resets the generator
with no other overhead.

Apart from resetting the generator,
