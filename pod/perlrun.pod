...
B<-D14> is equivalent to B<-Dtls>):
 ...
 536870912  K  Mutex locking/unlocking; enabled only when Configure
               was passed -Accflags=-DPERL_DEBUG_MUTEXES
...
enables all optional features and builtin functions, including experimental
ones (in the main compilation unit). See L<feature> and L<builtin>.
