...
The experimental C<case_match> feature provides a data-shape matching
alternative to the older C<given>/C<when> construct.  It is enabled with

    use feature 'case_match';

A C<case> evaluates its subject once and tries its C<match> arms in source
order.  The first matching arm runs; there is no fall-through.  The wildcard
data shape C<_> matches any value and is useful as a final arm:

    case ($value) {
        match ([$first, $second]) { use_pair($first, $second) }
        match (_)                 { use_other($value) }
    }
...
C<FloatVal>, or C<StrVal>:
    ...
    case (IntVal $value) {
        match (1) { say "integer one" }
    }

These forms apply integer, floating-point, or string conversion to the
subject once, before any arm is tested.  The type word is part of the case
