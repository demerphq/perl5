...
=head2 Experimental Pattern Matching

The experimental C<case_match> feature provides a pattern-oriented alternative
to the older C<given>/C<when> construct.  It is enabled with
...
pattern C<_> matches any value and is useful as a final arm:
...
and string literals remain distinct pattern kinds.

String patterns may contain literal fragments and one unpinned scalar
capture.  For example:
...
captures.  The concatenation is pattern syntax; it is not evaluated as an
...
Array and hash-reference patterns may contain nested patterns.  A trailing
C<...> makes an array or hash pattern open; without it, the pattern is exact.
Pattern bindings are tentative and are visible only after the complete
pattern and its optional guard succeed.  A guard is written inside the
C<match> parentheses:
...
available to patterns as a pinned value; it is not an assignment to an outer
