...
yyl_word_or_keyword(pTHX_ char *s, STRLEN len, I32 key, I32 orig_keyword, struct code c)
{
    ...
    case KEY_iterator_create:
        PREBLOCK(KW_ITERATOR_CREATE);

    case KEY_iterator_exhausted:
        PL_expect = XTERM;
        PL_bufptr = s;
        return REPORT(KW_ITERATOR_EXHAUSTED);

    case KEY_iterator_yield:
        PL_expect = XTERM;
        PL_bufptr = s;
        return REPORT(KW_ITERATOR_YIELD);
