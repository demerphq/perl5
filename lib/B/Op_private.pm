...
$bits{aeach}{0} = $bf[0];
@{$bits{aelem}}{5,4,1,0} = ($bf[11], $bf[11], $bf[1], $bf[1]);
...
@{$bits{casecoerce}}{7,6,5,4,3,2,1,0} = ($bf[10], $bf[10], $bf[10], $bf[10], $bf[10], $bf[10], $bf[10], $bf[0]);
...
@{$bits{concat}}{7,6,1,0} = ('OPpCONCAT_PATTERN', 'OPpCONCAT_NESTED', $bf[1], $bf[1]);
$bits{cond_expr}{0} = $bf[0];
@{$bits{connect}}{3,2,1,0} = ($bf[5], $bf[5], $bf[5], $bf[5]);
@{$bits{const}}{7,6,5,4,3,2,1} = ($bf[13], $bf[13], 'OPpCONST_BARE', 'OPpCONST_ENTERED', 'OPpCONST_STRICT', 'OPpCONST_SHORTCIRCUIT', 'OPpCONST_NOVER');
...
$bits{entercase}{0} = $bf[0];
$bits{entercasematch}{0} = $bf[0];
...
@{$bits{entersub}}{5,4,0} = ($bf[11], $bf[11], 'OPpENTERSUB_INARGS');
...
@{$bits{helem}}{5,4,1,0} = ($bf[11], $bf[11], $bf[1], $bf[1]);
...
$bits{leavecase}{0} = $bf[0];
$bits{leavecasematch}{0} = $bf[0];
...
$bits{lvavref}{0} = $bf[0];
@{$bits{lvref}}{5,4,0} = ($bf[12], $bf[12], $bf[0]);
...
$bits{refaddr}{0} = $bf[0];
@{$bits{refassign}}{5,4,1,0} = ($bf[12], $bf[12], $bf[1], $bf[1]);
$bits{refgen}{0} = $bf[0];
$bits{reftype}{0} = $bf[0];
...
@{$bits{rv2gv}}{6,5,4,2,0} = ('OPpALLOW_FAKE', $bf[11], $bf[11], 'OPpDONT_INIT_GV', $bf[0]);
$bits{rv2hv}{0} = 'OPpRV2HV_ISKEYS';
@{$bits{rv2sv}}{5,4,0} = ($bf[11], $bf[11], $bf[0]);
...
our %defines = (
    ...
    OPpCALLER_PKG            =>   1,
    OPpCALLER_SUB            =>   8,
    OPpCASECOERCE_MASK       => 254,
    OPpCONCAT_NESTED         =>  64,
    OPpCONCAT_PATTERN        => 128,
    OPpCONST_BARE            =>  32,
    OPpCONST_ENTERED         =>  16,
    OPpCONST_NOVER           =>   2,
...
$ops_using{OPpCONCAT_PATTERN} = $ops_using{OPpCONCAT_NESTED};
