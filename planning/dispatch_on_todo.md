# `dispatch`/`on` follow-up work

This document records the remaining work for the experimental
`dispatch` feature on the `yves/pattern_match` branch.  It is intentionally
...
- `use feature 'dispatch'` and the block-only form
  `dispatch (EXPR) { on (PATTERN) { ... } }`;
...
- unrestricted ordinary Perl guards inside `on (...)`;
...
- documentation in `pod/perldispatch.pod`, `pod/perlsyn.pod`, and
  `BLEAD-DELTA.md`;
- focused compiler/runtime coverage in `t/comp/dispatch.t`.
...
### 1. Give dispatch/on independent control-flow implementation

The current optree contains case-specific entry/leave operations layered onto
the older `given`/`when` machinery.  This must be replaced: `dispatch`/`on`
...
feature should not change the other, and dispatch/on must not inherit switch
fall-through, smartmatch, or other legacy control-flow behavior.

The preferred target is a dispatch-specific structure equivalent to:
...
- the dispatch behaves as a labelled block;
...
- nested dispatches do not corrupt the outer dispatch context;
- legacy `given`/`when` behavior remains unchanged.

Introduce dedicated dispatch operations and runtime context handling, and remove
the unnecessary given/when layers from dispatch/on.  Small, genuinely generic
...
- writes from an clause to the original subject;
...
- `pod/perldispatch.pod` in a beginner-friendly, CS-101 style;
